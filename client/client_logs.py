...
def write_train_logs(self, rnd, loss, acc, model_size, train_time, cyfer_time, decyfer_time):
    filename = f'../logs/{self.dataset}/{self.solution}/train_{self.technique}-{self.percentage}-{self.dirichlet_alpha} - {self.dataset} - {self.solution}.csv' 
...
def write_evaluate_logs(self, rnd, loss, acc, decyfer_time):
    filename = f'logs/{self.dataset}/{self.solution}/evaluate_{self.technique}-{self.percentage}-{self.dirichlet_alpha} - {self.dataset} - {self.solution}.csv' 
    os.makedirs(os.path.dirname(filename), exist_ok=True)
